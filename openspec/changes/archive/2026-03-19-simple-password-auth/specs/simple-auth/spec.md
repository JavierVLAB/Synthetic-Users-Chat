...
- **THEN** el frontend SHALL llamar a `POST /auth/login`, guardar el JWT en `localStorage` con clave `moeve-auth-token`, setear una cookie `auth-token` con el mismo valor para el middleware de Next.js, y redirigir a `/`
